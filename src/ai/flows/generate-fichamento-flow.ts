...
export const FichaLeituraSchema = z.object({
  ...
  palavrasChave: z.array(z.string()).describe("A list of 3-5 main keywords summarizing the content, in the language of the content."),
  ...
  citacoesRelevantes: z.array(z.string()).describe("One or two direct short quotes from the text that are particularly relevant or impactful. Must be exact quotes from the provided 'conteudo'."),
...
5.  **palavrasChave**: Gere uma lista de 3 a 5 palavras-chave principais que resumem o conteúdo do texto. As palavras-chave devem estar no mesmo idioma do "Conteúdo Principal do Texto".
...
7.  **citacoesRelevantes**: Extraia UMA ou DUAS citações DIRETAS e CURTAS do "Conteúdo Principal do Texto" que sejam particularmente relevantes, impactantes ou que exemplifiquem um ponto crucial. As citações devem ser literais do texto. Se não encontrar citações curtas e altamente relevantes, pode ser uma lista vazia.
8.  **comentariosAdicionais**: (Opcional) Adicione breves notas pessoais ou conexões com um tema principal de estudo. Se não tiver comentários, pode deixar este campo em branco.
...
const generateFichamentoFlow = ai.defineFlow(
  ...
  async (input) => {
    // Basic pre-processing for year if dataPublicacao is obviously just a year.
    let processedInput = {...input};
    if (input.dataPublicacao && /^\d{4}$/.test(input.dataPublicacao.trim())) {
        // If dataPublicacao is already just a year, ensure it's passed directly
        // The prompt also handles extraction, but this can help.
    }
    ...
    return {
      ...output,
      autor: output.autor?.trim() || undefined, // Ensure empty strings become undefined
