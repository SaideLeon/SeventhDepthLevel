...
export default function VSCodeCodeBlock({ 
  ...
  const codeToDisplay = initialCode || (propChildren ? String(propChildren).replace(/\n$/, '') : '');
  const langToDisplay = initialLanguage === '' ? 'plaintext' : initialLanguage;

  useEffect(() => {
    if (codeRef.current) {
      try {
        if (!codeRef.current.classList.contains('hljs') && !(codeRef.current as HTMLElement).dataset.highlighted) {
          hljs.highlightElement(codeRef.current);
          (codeRef.current as HTMLElement).dataset.highlighted = 'true';
        }
      } catch (e) {
        console.error("highlight.js error:", e);
      }
    }
  }, [codeToDisplay, langToDisplay]);
    ...
    <div className="bg-[#1e1e1e] rounded-xl overflow-hidden shadow-lg border border-gray-700/50 font-mono text-sm my-4 relative group" {...restProps}>
      ...
      <div className="p-4 overflow-x-auto text-white">
        <pre className="!p-0 !bg-transparent !whitespace-pre-wrap !break-words">
          <code ref={codeRef} className={`language-${langToDisplay}`}>
            {codeToDisplay}
          </code>
        </pre>
      </div>
